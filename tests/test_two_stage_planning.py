...
class TestTwoStageWorkflow:
        ...
        design = Design(
            ...
            architecture_decisions=[
                ArchitectureDecision(
                    title="Token Type", choice="JWT", alternatives=["sessions", "API keys"],
                    pros=["Stateless"], cons=["Revocation hard"],
                ),
                ArchitectureDecision(
                    title="Storage", choice="PostgreSQL", alternatives=["MongoDB"],
                    pros=["ACID"], cons=["Schema migrations"],
                ),
            ],
            api_contracts=["POST /auth/login -> {token}", "POST /auth/refresh -> {token}"],
    ...
    def test_plan_from_design_generates_coherent_plan(self, tmp_path):
        ...
        design = Design(
            ...
            architecture_decisions=[
                ArchitectureDecision(
                    title="Cache Backend", choice="Redis", alternatives=["Memcached"],
                    pros=["Rich data types"], cons=["Extra infra"],
                ),
            ],
...
class TestDesignGeneratorIntegration:
    ...
    def test_full_cli_workflow(self, tmp_path):
        ...
        r2 = runner.invoke(cli, [
            'plan',
            '--from-design', str(tmp_path / 'DESIGN.md'),
            '--project-path', str(tmp_path),
        ])
