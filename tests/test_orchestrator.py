...
def orchestrator(mock_config, tmp_path):
    ...
    (skills_dir / "fastapi.yaml").write_text(
        yaml.dump(
            [
                {
                    "name": "fastapi-expert",
                    "description": "Expert in {project_name}",
                    "category": "tech",
                }
            ]
        )
    )
    (skills_dir / "core.yaml").write_text(
        yaml.dump([{"name": "core", "description": "Core stuff", "category": "core"}])
    )
