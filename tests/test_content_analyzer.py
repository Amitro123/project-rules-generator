...
"""
        ...
        assert "header" in ' '.join(report.suggestions).lower() or \
               "structure" in ' '.join(report.suggestions).lower()
    ...
    def test_analyze_minimal_content(self, analyzer):
        ...
        assert any('detail' in s.lower() or 'brief' in s.lower() 
...
"""
        ...
        assert any('code' in s.lower() or 'example' in s.lower() 
                  for s in report.suggestions)
