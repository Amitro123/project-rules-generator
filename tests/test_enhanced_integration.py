...
class TestEndToEndEnhancedPipeline:
    ...
    def test_python_cli_project_full_flow(self, tmp_path):
        ...
        selected = matcher.match_skills(
            context['metadata']['tech_stack'], context
        )
    ...
    def test_self_project_analysis(self):
        ...
        has_react_learned = any(
            s.startswith('learned/react/') for s in selected
        )
...
class TestCodeExampleExtractor:
    ...
    def test_extract_click_patterns(self, tmp_path):
        ...
        assert any('click' in ex.get('reason', '').lower() or
                    'click' in ex.get('code', '').lower()
                    for ex in examples)
