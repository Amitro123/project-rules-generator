    ...
    try:
        ...
        if skills_manager and hasattr(skills_manager, "discovery") and skills_manager.discovery.project_skills_root:
            ...
            for skill_name, skill_data in all_discovered.items():
                ...
                if s_type == SkillScope.PROJECT:
                    enhanced_selected_skills.add(f"{SkillScope.PROJECT}/{skill_name}")
                    continue
                ...
                if project_type != "agent-skills":
                    ...
                    if s_path:
                        try:
                            ...
                            if s_type == SkillScope.LEARNED:
                                enhanced_selected_skills.add(f"{SkillScope.LEARNED}/{skill_name}")
                            elif s_type == SkillScope.BUILTIN:
                                enhanced_selected_skills.add(f"{SkillScope.BUILTIN}/{skill_name}")
        ...
        if project_type == "agent-skills":
            ...
            for p in project_path.rglob(SKILL_FILENAME):
...
def _llm_generate_skills(
    ...
    for skill_ref in sorted(enhanced_selected_skills):
        ...
        project_skill_path = SkillPathManager.output_skill_path(output_dir, SkillScope.LEARNED, skill_topic)
...
def _copy_skill_files(
    ...
    for skill_ref in sorted(enhanced_selected_skills):
        ...
        if skill_ref.startswith(f"{SkillScope.BUILTIN}/"):
            dest = SkillPathManager.output_skill_path(output_dir, SkillScope.BUILTIN, ref_name)
        elif skill_ref.startswith(f"{SkillScope.LEARNED}/"):
            dest = SkillPathManager.output_skill_path(output_dir, SkillScope.LEARNED, ref_name)
        else:
            continue
