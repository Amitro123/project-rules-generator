...
def create_rules(
    ...
    try:
        ...
        # Display metadata
        click.echo(f"\n📦 Detected Tech Stack: {', '.join(metadata.tech_stack)}")
        click.echo(f"🎯 Project Type: {metadata.project_type}")
        click.echo(f"⭐ Priority Areas: {', '.join(metadata.priority_areas)}")
        ...
        # Display rule count
        all_rules = content.count("- ✅") + content.count("- 🔴") + content.count("- 🟡") + content.count("- ⚪")
        click.echo(f"\n📊 Rules Summary:")
        click.echo(f"   - Total rules: {all_rules}")
        click.echo(f"   - Tech-specific: {sum(1 for tech in metadata.tech_stack if tech in content.lower())}")
        ...
        click.echo("\n✅ Cowork-quality rules generation complete!")
...
def _display_quality_report(quality, verbose: bool):
    ...
    if quality.conflicts:
        click.echo(f"\n⚠️  Rule Conflicts ({len(quality.conflicts)}):")
