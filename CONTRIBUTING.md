...
`--ide` is a `click.Choice` accepting only `antigravity` (or `none`) today —
unsupported values are rejected with a clear error rather than silently
ignored. If you add another IDE (e.g. a Cursor `.cursor/rules/` writer), extend
**both** the `--ide` Choice in `cli/analyze_cmd.py` **and** the
`_register_ide_rules` branch, mirroring the pattern: read from
...
### Policy: snapshot, not source of truth

`.clinerules/` is committed as a **snapshot for reviewers and dogfooding**, not as a source of truth. The actual source of truth is:

| Layer | Source of truth | Snapshot location |
|:------|:----------------|:------------------|
| **builtin** skills (ship with PRG) | `generator/skills/builtin/` | `.clinerules/skills/builtin/` |
| **learned** skills (reusable patterns) | `~/.project-rules-generator/learned_skills/` (user home) | `.clinerules/skills/learned/` |
| **project** skills (per-project, auto-generated) | regenerated by `prg analyze` on each run | `.clinerules/skills/project/` *(gitignored)* |
| `rules.md` / `clinerules.yaml` / `auto-triggers.json` | regenerated by `prg analyze` | gitignored |

What this means in practice:

- **Always edit the source-of-truth file, never the snapshot.** For a builtin skill, edit `generator/skills/builtin/<name>/SKILL.md`. For a learned skill, run `prg analyze . --create-skill <name>`.
- **The snapshot may lag behind source.** Maintainers refresh it before tagging a release; in-between, a small drift is expected. If you notice meaningful drift (a skill is in the source but missing from the snapshot, or vice-versa), file an issue rather than hand-editing.
- **Per-project artifacts are gitignored** (`rules.md`, `clinerules.yaml`, `auto-triggers.json`, `skills/project/`). These regenerate on every `prg analyze` run and would otherwise create noisy PR diffs.

> **Do not edit `.clinerules/` manually.** Edits to the snapshot will be overwritten the next time a maintainer refreshes it from source. Send PRs against the source-of-truth path instead.
...
- **CI note:** PRs trigger a [PR Agent](.github/workflows/pr-agent.yml) workflow that uses Gemini for AI-powered review. This requires a `GOOGLE_AI_STUDIO` repo secret — if you're a fork contributor, you can safely ignore this step; PR Agent will still run with reduced capabilities.
...
**File:** `cli/analyze_cmd.py` → the `--ide` Choice and `_register_ide_rules()`
**What:** Add `"cursor"` to the `--ide` `click.Choice`, then add a branch to
`_register_ide_rules()` that writes `.cursor/rules/<project>.md` mirroring the
`antigravity` branch. The pattern is already there — copy it, change the
target path, add a test in `tests/test_ide_flag.py`.
