...
class CoworkRulesCreator:
    ...
    def _detect_tech_stack(
        ...
        """Auto-detect tech stack from README and context."""
        tech_keywords = {
    ...
    def _generate_rules(
        ...
        # 3. Testing rules
        if "has_tests" in metadata.detected_signals:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if hotspots:
                    antipatterns.append(
                        Rule(
                            f"⚠️ Hot spots detected: {', '.join(hotspots[:3])} - consider refactoring",
                            priority="Medium",
