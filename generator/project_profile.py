...
# --- Phase 4d: canonical skill-ref dedup ----------------------------------
#
# Replaces three scattered dedup sites that each used a different key
# scheme and were the documented source of nbug.md (pydantic-validation
# in two scopes) + Bug-1 ordering drift:
#
#   1. _parse_skill_refs (above) — dedupes (scope, terminal_name) for the
...
# --- Phase 4a: tag ∩ tech_stack filter for skill selection -----------------
#
...
# SKILL.md frontmatter lives, how tags are parsed, or which scope is which.
...
def filter_skills_by_tech_overlap(
    selected_refs: "Set[str] | FrozenSet[str]",
    tech_stack: "Set[str] | FrozenSet[str]",
