...
class ProjectAnalyzer:
    ...
    def _analyze_structure(self) -> Dict:
        ...
        exclude = {'.git', 'node_modules', 'venv', '__pycache__', '.pytest_cache', 'dist', 'build', '.idea', '.vscode', 'skills'}
        main_dirs = [
    ...
    def _get_key_files(self) -> Dict[str, str]:
        ...
        # Config files
        for filename in ['.env.example', 'config.py', 'settings.py', 'vite.config.ts', 'vite.config.js']:
            file_path = self.project_path / filename
            if file_path.exists():
                try:
                    content = file_path.read_text(encoding='utf-8', errors='replace')
