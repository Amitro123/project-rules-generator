"""CoworkRulesCreator — generates high-quality, project-specific coding rules."""
...
from generator.utils.quality_validators import RulesQualityValidator
...
class CoworkRulesCreator(ArtifactGenerator):
    ...
    def _detect_tech_stack(self, readme_content: str, enhanced_context: Optional[Dict]) -> List[str]:
        ...
        if enhanced_context:
            context_tech = enhanced_context.get("metadata", {}).get("tech_stack", []) or enhanced_context.get(
                "project_data", {}
            ).get("tech_stack", [])
            ...
            for line in response.splitlines():
                ...
                if line.upper().startswith("DO:"):
                    ...
                    rules_by_category["Coding Standards"].append(
                        Rule(content=content, priority="High", category="Coding Standards", source="llm_fallback")
                    )
    ...
    def _generate_rules(
        ...
        if has_test_signal:
            rules_by_category["Testing"].extend(self._generate_testing_rules(metadata))

        rules_by_category["General"].extend(self._generate_generic_rules(metadata))
    ...
    def _generate_testing_rules(self, metadata: RulesMetadata) -> List[Rule]:
        ...
        if test_framework:
            rules.append(Rule(f"Run {test_framework} before committing", priority="High", category="Testing"))
            rules.append(Rule("Add tests for all new features", priority="High", category="Testing"))
            rules.append(Rule("Maintain test coverage above 90%", priority="Medium", category="Testing"))
    ...
    def _generate_generic_rules(self, metadata: RulesMetadata) -> List[Rule]:
        ...
        return [
            Rule("Follow existing project structure and naming conventions", priority="High", category="General"),
            Rule("Add docstrings to all public functions/classes", priority="Medium", category="General"),
            Rule("Don't commit secrets, API keys, or .env files", priority="High", category="General"),
        ]
